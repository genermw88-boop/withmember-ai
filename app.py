        ...
        if not store or not reg or not men:
            ...
            with st.spinner("네이버 상권 데이터 수집 및 소개글을 생성 중입니다... (약 10초 소요)"):
                ...
                if msg == "success":
                    ...
                    # 💡 프롬프트 고도화: 극강의 자연스러움, 감성 블로그 톤앤매너, 네이버 로직 완벽 대응
                    prompt = f"""
                    매장명: '{store}'
                    지역: '{reg}'
                    주력메뉴: '{men}'
                    ...
                    2. [제목]: 20~30자 내외로 이목을 끄는 매력적인 제목을 작성하세요. (이모티콘 포함)
                    3. [본문 톤앤매너]: 딱딱한 홍보글은 절대 금지! 진짜 방문객이나 친한 단골 손님에게 이야기하듯 **친근하고, 자연스럽고, 부드러운 감성 블로그/인스타그램 SNS 스타일**로 작성하세요. ("~했어요", "~해볼까요?" 등 부드러운 대화체 사용)
                    4. [네이버 SEO 최적화]: 위 [참고 타겟 키워드 목록] 중 **가장 잘 어울리는 2~3개의 핵심 키워드만 선택**하여 문맥에 100% 자연스럽게 녹여내세요. (키워드를 억지로 구겨 넣으면 네이버 로직에서 누락되니 가장 자연스러운 문맥을 우선시하세요.)
                    5. 글자 수: **공백 포함 200자 내외(3~5문장)**로, 너무 짧지도 않고 지루하지도 않게 작성하세요.
                    6. 글의 흐름: '공감/따뜻한 인사말 ➔ 매장이나 메뉴의 숨겨진 매력 어필 ➔ 부담 없이 편안한 방문 유도' 순으로 물 흐르듯 이어지게 쓰세요.
                    7. 문장 사이사이에 감성적인 이모티콘 2~3개를 적절히 배치하세요.
