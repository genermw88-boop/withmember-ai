        ...
        if not store or not reg or not men:
            ...
            with st.spinner("네이버 상권 데이터 수집 및 소개글을 생성 중입니다... (약 10초 소요)"):
                ...
                if msg == "success":
                    ...
                    # 💡 프롬프트 수정: 친근하고 부드러운 블로그/SNS 스타일, 200자 이내
                    prompt = f"""
                    매장명: '{store}'
                    지역: '{reg}'
                    주력메뉴: '{men}'
                    ...
                    3. [본문]: 단골 손님과 소통하듯 **친근하고 부드러운 SNS나 블로그 스타일**로 작성하세요. 너무 딱딱하거나 홍보성 짙은 말투는 피해주세요.
                    4. [네이버 플레이스 SEO 로직 최적화] 검색 노출에 유리하도록 위 [참고 타겟 키워드 목록] 중 **2~3개의 핵심 키워드를 선택**하여 문맥에 억지스럽지 않게 100% 자연스럽게 녹여내세요. (키워드 반복이나 나열은 절대 금지!)
                    5. 글자 수는 **공백 포함 최대 200자 이내**로 작성하세요. 너무 길어지지 않도록 핵심적인 인사말과 메뉴의 매력 어필, 방문 유도만 깔끔하게 담아주세요.
                    6. 세련된 이모티콘 2~3개를 적절한 위치에 배치하세요.
                    7. [강력 경고] 제공된 지역('{reg}')과 메뉴('{men}') 외에 다른 지역명이나 엉뚱한 메뉴는 절대 지어내서 적지 마세요.
                    8. [강력 경고] 글 마지막에 해시태그(#)를 달거나, 추천 키워드 목록을 따로 출력하지 마세요. 오직 본문 문장 내에만 키워드가 들어가야 합니다.
                    """
