        ...
        if not store or not reg or not men:
            ...
            with st.spinner("네이버 상권 데이터 수집 및 소개글을 생성 중입니다... (약 10초 소요)"):
                ...
                if msg == "success":
                    ...
                    # 💡 프롬프트 초강력 고도화: 완벽한 레퍼런스(예시) 제공으로 원하는 톤과 길이 100% 매칭
                    prompt = f"""
                    매장명: '{store}'
                    지역: '{reg}'
                    주력메뉴: '{men}'
                    ...
                    3. [본문 톤앤매너]: 동네 주민에게 맛집을 소개하듯 **친근하고 부드러운 블로그/SNS 스타일**로 작성하세요. ("~했어요", "~제격이에요", "~즐겨보세요!" 등 자연스럽고 따뜻한 말투)
                    4. [네이버 SEO 최적화]: 위 [참고 타겟 키워드 목록] 중 **가장 잘 어울리는 2~3개의 핵심 키워드만 선택**하여 문장에 전혀 억지스럽지 않게 녹여내세요.
                    5. 글자 수: **공백 포함 200자 ~ 250자 내외(3~5문장)**로 작성하여 예시와 비슷한 길이를 유지하세요.
                    6. 매장명 강조: 본문 내에서 매장명('{store}')을 **굵게(볼드처리)** 강조해주세요. 
                    7. [강력 경고] 제공된 정보 외의 내용(없는 메뉴, 타 지역)은 절대 지어내지 마세요. 글 마지막에 해시태그(#)를 달거나 추천 키워드 목록을 따로 출력하지 마세요.

                    [가장 이상적인 출력 예시] - 아래 예시의 감성과 길이를 완벽하게 모방하여 작성하세요!
                    [제목] 명장동에서 제대로 된 진한 갈비 맛을 찾고 계신다면? 🍖✨
                    [본문] 부산 동래구 명장동에서 깊은 국물의 갈비탕과 매콤달콤 부드러운 갈비찜으로 유명한 **'명장본가 진갈비탕'**을 소개합니다! 이미 동네 주민들 사이에서는 소문난 맛집인데요. 고기가 워낙 부드럽고 국물이 진해서 든든한 점심 식사는 물론, 가족 외식이나 모임 장소로도 제격이에요. 오늘 저녁, 정갈하고 맛있는 밥 한 끼가 생각나신다면 명장동 추천 맛집 **'명장본가 진갈비탕'**에서 특별한 식사를 즐겨보세요! 🍲🥢
                    """
