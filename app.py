        ...
        if not store or not reg or not men:
            ...
            with st.spinner("네이버 상권 데이터 수집 및 소개글을 생성 중입니다... (약 10초 소요)"):
                ...
                if msg == "success":
                    ...
                    # 💡 프롬프트 대폭 수정: 키워드 1~2개만 핵심적으로 사용하고, 글자 수를 줄이도록 변경
                    prompt = f"""
                    매장명: '{store}'
                    지역: '{reg}'
                    주력메뉴: '{men}'
                    ...
                    5. 문구가 너무 길어지지 않도록, 핵심만 담아 간결하게(공백 포함 100자 ~ 130자 내외) 작성하세요.
                    6. 세련된 이모티콘 2~3개를 배치하세요.
    ...
    if submit_review:
        if not review_content:
            ...
            with st.spinner("정성스러운 답글을 작성 중..."):
                # 💡 리뷰 프롬프트 수정: 완벽한 존댓말 적용 및 단답형 금지 조건 추가
