        ...
        if not store or not reg or not men:
            ...
            with st.spinner("네이버 상권 데이터 수집 및 소개글을 생성 중입니다... (약 10초 소요)"):
                ...
                if msg == "success":
                    ...
                    # 💡 프롬프트 수정: 너무 짧은 단답형 방지 및 2~3문장의 정성스러운 분량(100~130자) 유도
                    prompt = f"""
                    매장명: '{store}'
                    지역: '{reg}'
                    주력메뉴: '{men}'
                    ...
                    3. [본문]: 위 [참고 타겟 키워드 목록] 중 **가장 문맥에 잘 어울리는 1~2개의 핵심 키워드만 선택**하여 자연스럽게 문장에 녹여내세요. (모든 키워드를 억지로 넣지 마세요!)
                    4. 인스타그램 감성 맛집 블로거처럼 물 흐르듯 자연스럽게 작성하세요.
                    5. 너무 짧게 끝나지 않도록 주의하세요! 고객을 맞이하는 정성스러운 인사말, 메뉴의 맛/매력 어필, 따뜻한 방문 유도 멘트를 포함하여 **공백 포함 100자 ~ 130자 내외(2~3문장)**로 알차고 정성스럽게 작성하세요.
                    6. 세련된 이모티콘 2~3개를 적절한 위치에 배치하세요.
                    7. [강력 경고] 제공된 지역('{reg}')과 메뉴('{men}') 외에 다른 지역명이나 엉뚱한 메뉴는 절대 지어내서 적지 마세요.
