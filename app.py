...
# Streamlit Secrets에서 가져오기 (설정 안 되어 있으면 사이드바 입력창 활성화)
...
with tab1:
    ...
    with st.form("intro_form"):
        ...
        with c1: reg = st.text_input("지역", placeholder="강남역")
        with c2: cat = st.text_input("업종", placeholder="삼겹살집")
        with c3: men = st.text_input("주력메뉴", placeholder="숙성 삼겹살")
    ...
    if submit_intro:
        with st.spinner("데이터 분석 중..."):
            ...
            if kws:
                ...
                prompt = f"지역:{reg}, 업종:{cat}, 메뉴:{men}, 키워드:{','.join(kws)}를 포함해서 플레이스 소개글 50자 내외로 써줘. 첫 문장에 1위 키워드 배치해."
...
with tab2:
    ...
    if submit_review:
        with st.spinner("정성스러운 답글을 작성 중..."):
            prompt = f"다음 리뷰에 대해 친절하고 정중한 사장님 톤으로 답글을 써줘. 매장 이름은 '위드멤버 제휴점'으로 가정해. 리뷰내용: {review_content}"
